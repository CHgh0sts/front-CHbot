...
import { patchPartyPresetBodySchema } from '@/lib/party-preset-schema';
import {
...
export async function PATCH(req: Request, context: RouteCtx) {
  ...
  const parsed = patchPartyPresetBodySchema.safeParse(json);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const data: Prisma.PartyConfigPresetUpdateInput = {};

  if (body.name !== undefined) {
    data.name = body.name;
  }
  if (body.composition !== undefined) {
    data.composition = body.composition as object;
  }

  if (body.roleImageOverrides !== undefined) {
    ...
    const checked = validateRoleOverridesInput(body.roleImageOverrides);
    if ('error' in checked) {
      return NextResponse.json({ error: checked.error }, { status: 400 });
    }
    data.roleImageOverrides =
  ...
  const fresh = await prisma.partyConfigPreset.findUnique({
    where: { id: existing.id },
    select: {
      publicCode: true,
      name: true,
  ...
  return NextResponse.json({
    publicCode: fresh!.publicCode,
    name: fresh!.name,
    composition: fresh!.composition,
    roleImageOverrides: readRoleOverridesFromDb(fresh!.roleImageOverrides),
  });
}
...
export async function DELETE(_req: Request, context: RouteCtx) {
  ...
  const row = await prisma.partyConfigPreset.findFirst({
    where: { publicCode: code, userId: session.user.id },
    select: { id: true },
  });
  if (!row) {
  ...
  await prisma.partyConfigPreset.delete({ where: { id: row.id } });
